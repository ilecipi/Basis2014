...
\end{lstlisting}                                                                                                 
\subsection*{Analisi}
La costruzione dell'heap avviene in tempo lineare. Il resto dell'algoritmo prevede $n$ volte il metodo versickere, quindi in complesso\todo{??} l'algoritmo richiede $O(n \log (n))$ comparazioni e spostamenti nel peggior caso. Da notare che non occupa spazio extra, quindi è completamente ``in situ''. L'unico problema è che non è stabile, vale a dire che la posizione relativa di chiavi con lo stesso valore inizialmente adiacenti può cambiare. 
...
\todo[inline]{Dove c'è scritto ``scegliere pivot v'' non dovrebbe essere un commento??}
\begin{lstlisting}[tabsize=\lstIndent]
trova (int l,r,i)
	int m,v
	if r>l
...
report(Node p, Point x)
	...
	if (p ist ein Blatt)
		...
		if (p ha un figlio destro d) && (x is in I(d))
			report(d,x)				
\end{lstlisting}
\todo{In add non dovrebbe essere il contrario e sovrapporsi!?}
