...
\begin{center}
\includegraphics[scale=0.2]{Figures/programmersViewCoreAndData.jpg}
\end{center}
  ...
  \item Extract from Java API
    ...
    \item Classes and objects
     ...
     \item \textbf{Object}: An entity that combines data and behavior.
     	...
     	\end{itemize}
     	\item Inheritance\\
     	Even thought there are systematic differences, different kinds of objects often have a certain amount in common with each other.\\ Classes \textit{inherit} commonly used state and behaviour from other classes, but specialized behaviour and states further \todo{WTF??}
	...
	\begin{itemize}
		...
		\item Building thread-safe classes
			\begin{itemize}
				...
				\item Immutable it thread safe always after initialization
...
lock:
	...
	unlock:
        my_t.set(false);
\end{lstlisting}
Two AtomicBooleans (one per thread) and an AtomicInteger which decides which thread will be selected. This only works for two threads. The entities \inline{my_t}, \inline{other_t} and \inline{me} depends on the thread: for thread0 \inline{my_t=t0, other_t=t1, me=0} and for thread1 is the opposite.
\end{itemize}
...
\end{itemize}
	\item GPGPU Processor Terminology
		\begin{itemize}
			\item SPA (device)
				\begin{itemize}
					\item \textbf{S}treaming \textbf{P}rocessor \textbf{A}rray
				\end{itemize}
			\item SM
				\begin{itemize}
					\item \textbf{S}treaming \textbf{M}ultiprocessor 
					\item Multi-threaded processor core
					\item Fundamental processing unit for GPU thread block
				\end{itemize}
			\item SP
				\begin{itemize}
					\item \textbf{S}treaming \textbf{P}rocessor
					\item Scalar ALU for a single CUDA thread
				\end{itemize}
			\begin{center}
       			\includegraphics[scale=1]{Figures/GPGPUProcessor.jpg}
        	\end{center}
		\end{itemize}
	\item \textbf{CUDA} -`Computer Unified Device Architecture'
		\begin{itemize}
			\item General purpose programming model
				...
				\end{itemize}
			\item Targeted software stack
				\begin{itemize}
					\item Computer oriented drivers, language, and tools
				\end{itemize}
