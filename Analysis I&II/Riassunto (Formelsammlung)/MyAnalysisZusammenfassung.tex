...
\fancyhead[R]{Sandro Marcon, B.\ Sc.\ ETH Informatik, Legi-Nr.:  13-932-868 S. \thepage}
...
\begin{itemize}
\item $\lim_{n\rightarrow \infty} \left( 1+\frac{a}{n} \right)^n=e^a$
\item $\lim_{n\rightarrow 0} \frac{\sin(n)}{n}=1$
\item $\lim_{n\rightarrow 0} \frac{1-\cos(n)}{n}=0$
\item $\lim_{n\rightarrow 0} \frac{1-\cos(n)}{n^2}=\frac{1}{2}$
\item $\lim_{n\rightarrow 0} \frac{\log_a(1+n)}{n}=\frac{1}{\log(a)}$
\item $\lim_{n\rightarrow 0} \frac{a^n-1}{n}=\log(a)$
\end{itemize}
...
\item Ist $f$ stetig auf einem kompakten Intervall, dann ist $f$ gleichmässig stetig.
\end{enumerate}

\paragraph{Kompaktheit}
Eine Teilmenge $K\subset \mathbb{R}$ heisst kompakt, falls jede Folge von Punkte aus $K$ einen Häufungspunkt in $K$ besistzt. Kompakt impliziert beschränkt, das heisst ein kompaktes Intervall besitzt ein Maximum und ein Minimum.

\paragraph{Satz 4.12}
Sei $f: K \rightarrow \mathbb{R}^n$, $K$ kompakt. Dann ist $f(K)$ kompakt und $f$ nimmt Supremum und Infimum an. Das heisst $$\exists c_{+}, c_{-} \in K : f(c_{-})\leq f(x) \leq f(c_{+}) \forall x \in K$$
...
\paragraph{Folgerung} 
Sei $f$ auf dem Intervall $[a,b]$ differenzierbar. Falls die Funktion $x$ Nulstellen im Intervall besitzt, so muss $f'$ mindestens $x-1$ Nulstellen haben.
...
\paragraph{Jensen's Ungleichung}
Sei $f:]a,b[\rightarrow \mathbb{R}$ konvex $\forall x_1, ..., x_n \in ]a,b[$ und $t_1, ..., t_n \in [0,1]$ mit $\sum t_i=1$. Dann gilt
$$f\left( \sum_{i=1 }^n t_i x_i \right) \leq \sum_{i=1}^n t_i f(x_i)$$
Aus diese Ungleichung folgt dass $\left(\prod_{i=1}^n x_i \right)^{\frac{1}{n}} \leq \frac{1}{n} \sum_{i=1}^n x_1$.
Im Fall von $n=2$ gilt: $\frac{x_1+x_2}{2} \geq \sqrt{x_1x_2}$. 

\paragraph{Bemerkung} Die Exponetialfunktion ist konvex. 
...
Es ist also:
\[ T_m f(x;x_0) = \sum_{i=0}^{m} \dfrac{f^{(n)}(x_0)}{n!}(x-x_0)^n = f(x_0) + \dfrac{f'(x_0)}{1} + \dfrac{f''(x_0)}{2} + \cdots\]

\paragraph{Restglied}

\[ R_{m}(x) = \frac{f^{(m+1)}(a)}{(m+1)!}(x-x_0)^{m+1} \]
für ein a zwischen $x$ und $x_0$.
...
\paragraph*{Beispiel: Hauptsatz und Kettenregel}
Berechnen sie die Ableitung der Funktion $f(x)=\int_0^{x^3} \cos(t)^3 dt$. Man kann die Funktion $f$ in zwei Funktionen wie folgt zerlegen: $g(s)=\int_0^s \cos(t)^3 dt$ und $h(x)=x^3$. Dann aus der Kettenregel und der Hauptsatz der Intergralrechnung folgt:
$$f'(x)=(g(h(x))'=g'(h(x))h'(x)= 3\cos(x^3)^3 x^2$$
...
\paragraph{Bogenlänge und Volumina eines Kurvenstücks}
Sei $f$ eine differenzierbare Funktion auf dem Intervall $[a,b]$. Dann ist die Bogenlänge gegeben durch:
$$L=\int_a^b \sqrt{1+(f'(x))^2} dx$$
Das Volumen der Rotation der Figur auf der $0x$ Achse ist gegeben durch:
$$V=\pi \int_a^b (f(x))^2 dx$$
Die laterale Oberfläche ist: $A=2\pi \int_a^b f(x)\sqrt{1+(f'(x))^2} dx$
...
Wenn man ein komplexes $\lambda$ erhaltet, kann man zur Basis $e^{\alpha x}(c_1\cos(\beta x)+c_2\sin(\beta x))$ wählen, wobei $\alpha=\Re(\lambda) \mbox{ und } \beta=\Im(\lambda)$
...
\paragraph*{Anwendung} Sei eine Funktion wie folgt gegeben: $f(t)=u(x(t),y(t))$. 
Dann ist $\frac{df}{dt}(t)=u_x (x(t),y(t))x'(t)+ u_y (x(t),y(t))y'(t)$.

\paragraph*{Beispiel}
Berechnen sie die Ableitung der Funktion $g(x)=\int_0^x \sin(xt^2)dt$. Wir benutzen, dass $g(x)=f(x,x)=f(x(x),y(x))$. Dann
\begin{align*}
\frac{dg}{dx}(x)&=\frac{df}{dx}(x(x),y(x))=f_x(x,x)\frac{d}{dx}(x)+f_y(x,x)\frac{d}{dx}(x) \\
&= \sin(x^3)+\int_0^x t^2\cos(xt^2)dt
\end{align*}
...
\subsubsection{Niveaulineen}
$f(x,y)=c$ auflösen, wobei $c$ die Koordinate auf der Achse Oz entspricht.

\subsubsection{Lagrangian-Multiplikatoren}
Gegeben sei eine Funktion $f$. Wir suchen die globale Extrema der Funktion auf dem Gebiet $g$. Dann erhalten wir das System:
...
Im Fall eines 1-Norm $\lambda(x)= f_1 dx_1+ f_2 dx_2 + ... + f_n x_n$ ist das Wegintegral definiert durch:
$$\int_{\gamma} \lambda = \int_a^b \lambda (\gamma (t)) \gamma '(t) dt$$
...
\begin{enumerate}[label={(}\arabic*{)}]
 ...
 \item $\cos(2x) = \cos(x)^2 - \sin(x)^2 = 2\cos(x)^2 - 1 = 1 - 2\sin(x)^2$
 \item $\tan(2x) = \frac{2\tan(x)}{1-tan^2(x)}$
 ...
 \item $\mathrm{sen}\,\alpha+\mathrm{sen}\,\beta=2\,\mathrm{sen} \frac {\alpha+\beta}{2}\cos \frac {\alpha-\beta}{2}$
 \item $\mathrm{sen}\,\alpha-\mathrm{sen}\,\beta=2\cos \frac {\alpha+\beta}{2} \,\mathrm{sen} \frac {\alpha-\beta}{2}$
 \item $\cos\alpha-\cos\beta=-2 \,\mathrm{sen} \frac {\alpha+\beta}{2} \,\mathrm{sen} \frac {\alpha-\beta}{2}$
 \item $\tan\alpha\pm\tan\beta=\frac {\mathrm{sen}(\alpha\pm\beta)} {\cos\alpha \cos\beta}$
\end{enumerate}
...
\begin{enumerate}[label={(}\arabic*{)}]
 ...
 \item $\tanh(x) := \frac{\sinh(x)}{\cosh(x)} = 1 - \frac{2}{e^{2x}+1}$
...
\begin{enumerate}[label={(}\arabic*{)}]
 ...
 \item $\tan(x) \rightarrow \frac{1}{(\cos(x))^2}=1+\tan^2(x)$
...
\begin{tabular}{lll}
 \textbf{Körper} & $V$ & $A$\\\hline
 Kugel & $\frac{4\pi}{3}r^3$ & $4\pi r^2$\\
 Ellipsoid & $\frac{4\pi}{3}abc$ \\
 Zylinder & $\pi r^2h$ & $2 \pi r^2 + 2 \pi rh$\\
 Pyramide & $\frac{1}{3}Gh$ \\
 Kegel & $\frac{\pi}{3}r^2h$ & $\pi r (r+a) $\\
\end{tabular}

Wobei $G$ die Oberfläche der Basis entspricht.
...
\end{pmatrix}
\]
Wobei $a,b$ die Halbachsen sind.
Oberfläche: $A=\pi ab$

\paragraph{Kreis}
\begin{itemize}
\item Umfang=$2\pi r$
\item Oberfläche=$r^2 \pi$
\end{itemize}
